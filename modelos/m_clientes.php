<?php 
	...
	class ModeloClientes
	{
		...
		{
			 $stmt2 = Conexion::conectar()->prepare("SELECT * FROM persona INNER JOIN tienda ON persona.idPersona = tienda.Persona_idPersona INNER JOIN sector ON persona.Sector_idSector  = sector.idSector INNER JOIN zonas ON sector.Zonas_idZonas = zonas.idZonas INNER JOIN ciudad ON sector.Ciudad_idCiudad = ciudad.idCiudad   WHERE  persona.TipoUsuario_idTipoUsuario = :tipoUsuario AND zonas.descripcion = :zona AND ciudad.idCiudad = :ciudad");
